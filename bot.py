...
# Базовые настройки входа
BASE_MIN_SCORE = 4.0
MIN_SCORE = 4.0
MAX_SCORE = 7.0
BASE_MIN_SCORE_DIFF = 1.8
MIN_SCORE_DIFF = 1.8
...
# Защита от затянутых позиций
MAX_HOLD_HOURS = 6
RED_NEWS_DROP = 0.01  # 1% за 5 минут
RED_NEWS_VOL = 2.0    # объём ×2
RED_NEWS_BLOCK = 15 * 60  # 15 минут блок

# Сессионный множитель
NIGHT_HOURS = (1, 6)  # 01:00-06:00 UTC

# Ликвидации
LIQ_THRESHOLD = 5.0  # миллионов
...
@app.route("/")
def home():
    return f"OKX Scalp Bot v4.8 [{datetime.now(timezone.utc).strftime('%d.%m.%Y %H:%M')} UTC"
...
cache = {
    ...
    "grayscale_premium": {"value": 0.0, "ts": 0},
    "defi_tvl_ratio": {"value": 0.0, "ts": 0},
    "github_activity": {"value": 0, "ts": 0},
    "coinglass_oi": {"value": 0, "change": 0, "ts": 0},
    "cme_gap": {"value": 0.0, "ts": 0},
    "stablecoin_mint": {"value": 0, "ts": 0},
    "btc_seasonality": {"value": 0.0, "ts": 0},
    "large_tx_count": {"value": 0, "ts": 0},
}
...
def load_stats():
    ...
    try:
        ...
        log.info("Нет сохранённой статистики")
...
def load_xgb_model():
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            log.warning(f"Модель не найдена (статус {r.status_code})")
    except Exception as e:
        log.warning(f"Модель не загружена: {e}")
    return False
...
def save_trade_to_db(direction, score, L, S, metrics, result):
    ...
    try:
        ...
        """, (
            ...
            metrics.get("liq_diff", 0), metrics.get("gas_price", 20),
            metrics.get("btc_dom_change", 0), metrics.get("adx", 0),
            metrics.get("ema50_200_diff", 0), metrics.get("obv_divergence", 0),
            metrics.get("whales_detected", 0), metrics.get("news_sentiment", 0),
            metrics.get("funding_avg", 0), result
        ))
        conn.commit()
        cursor.close()
        conn.close()
...
def okx_place_order(direction, entry, sl, tp):
    ...
    save_active_positions()
    if not sl_ok:
        log.error("❌ SL НЕ УСТАНОВЛЕН после 3 попыток")
    if not tp_ok:
        log.error("❌ TP НЕ УСТАНОВЛЕН после 3 попыток")
...
def get_news_sentiment():
    ...
    try:
        ...
        if df is not None:
            vol_change = df["volume"].iloc[-1] / df["volume"].mean() if df["volume"].mean() > 0 else 1
            price_change = (df["close"].iloc[-1] - df["close"].iloc[-6]) / df["close"].iloc[-6] * 100
            sentiment = (vol_change - 1) * 0.5 + price_change * 0.1
            sentiment = max(-1.0, min(1.0, sentiment))
            cache["news_sentiment"] = {"value": sentiment, "ts": now}
            log.info(f"📰 News (эмуляция): {sentiment:+.2f}")
            return sentiment
    except Exception as e:
...
# НОВЫЕ МЕТРИКИ v4.8
# ══════════════════════════════════════════════
def get_grayscale_premium():
    now = time.time()
    if now - cache["grayscale_premium"]["ts"] < 600:
        return cache["grayscale_premium"]["value"]
    try:
        r = requests.get("https://api.coingecko.com/api/v3/simple/price?ids=ethereum-trust&vs_currencies=usd&include_market_cap=true", timeout=10)
        if r.status_code == 200:
            data = r.json()
            trust_price = data.get("ethereum-trust", {}).get("usd", 0)
            if trust_price > 0:
                eth_price = float(requests.get(f"https://api.binance.com/api/v3/ticker/price?symbol={SYMBOL_BN}", timeout=8).json()["price"])
                premium = (trust_price - eth_price) / eth_price * 100
                cache["grayscale_premium"] = {"value": premium, "ts": now}
                log.info(f"🏦 Grayscale premium: {premium:+.2f}%")
                return premium
    except:
        pass
    return 0.0

def get_defi_tvl_ratio():
    now = time.time()
    if now - cache["defi_tvl_ratio"]["ts"] < 600:
        return cache["defi_tvl_ratio"]["value"]
    try:
        eth_r = requests.get("https://api.coingecko.com/api/v3/simple/price?ids=ethereum&vs_currencies=usd&include_market_cap=true", timeout=10).json()
        eth_mcap = eth_r["ethereum"]["usd_market_cap"]
        defi_r = requests.get("https://api.llama.fi/protocols", timeout=10).json()
        eth_tvl = sum(p.get("tvl", 0) for p in defi_r if p.get("chain") == "Ethereum")
        if eth_mcap > 0 and eth_tvl > 0:
            ratio = eth_tvl / eth_mcap
            cache["defi_tvl_ratio"] = {"value": ratio, "ts": now}
            log.info(f"📊 DeFi TVL/ETH: {ratio:.4f}")
            return ratio
    except:
        pass
    return 0.0
...
def get_github_activity():
    now = time.time()
    if now - cache["github_activity"]["ts"] < 3600:
        return cache["github_activity"]["value"]
    try:
        r = requests.get("https://api.github.com/repos/ethereum/go-ethereum/commits?per_page=1", timeout=10)
        if r.status_code == 200:
            last_commit = r.json()[0]["commit"]["committer"]["date"]
            last_time = datetime.fromisoformat(last_commit.replace("Z", "+00:00"))
            hours_ago = (datetime.now(timezone.utc) - last_time).total_seconds() / 3600
            activity = 1 if hours_ago < 24 else 0
            cache["github_activity"] = {"value": activity, "ts": now}
            return activity
    except:
        pass
    return 0
...
def get_coinglass_oi():
    now = time.time()
    if now - cache["coinglass_oi"]["ts"] < 300:
        return cache["coinglass_oi"]["value"], cache["coinglass_oi"]["change"]
    try:
        r = requests.get("https://open-api.coinglass.com/public/v2/open_interest?symbol=ETH", timeout=10)
        if r.status_code == 200:
            data = r.json().get("data", {})
            oi = float(data.get("openInterest", 0))
            prev = cache["coinglass_oi"]["value"]
            chg = ((oi - prev) / prev * 100) if prev > 0 else 0
            cache["coinglass_oi"] = {"value": oi, "change": chg, "ts": now}
            return oi, chg
    except:
        pass
    return 0, 0
...
def get_cme_gap():
    now = time.time()
    if now - cache["cme_gap"]["ts"] < 3600:
        return cache["cme_gap"]["value"]
    try:
        day = datetime.now(timezone.utc).weekday()
        if day in [5, 6]:
            cache["cme_gap"] = {"value": 0.0, "ts": now}
            return 0.0
        df = get_klines("ETHUSDT", "1h", 48)
        if df is not None:
            df["date"] = pd.to_datetime(df["time"], unit="ms")
            friday_close = df[df["date"].dt.weekday == 4]["close"].iloc[-1] if len(df[df["date"].dt.weekday == 4]) > 0 else 0
            if friday_close > 0:
                current = float(requests.get(f"https://api.binance.com/api/v3/ticker/price?symbol={SYMBOL_BN}", timeout=8).json()["price"])
                gap = (current - friday_close) / friday_close * 100
                cache["cme_gap"] = {"value": gap, "ts": now}
                return gap
    except:
        pass
    return 0.0

def get_stablecoin_mint():
    now = time.time()
    if now - cache["stablecoin_mint"]["ts"] < 600:
        return cache["stablecoin_mint"]["value"]
    try:
        r = requests.get("https://api.coingecko.com/api/v3/simple/price?ids=tether&vs_currencies=usd&include_24hr_vol_change=true", timeout=10)
        if r.status_code == 200:
            vol_change = r.json()["tether"].get("usd_24h_vol_change", 0)
            signal = 1 if vol_change and vol_change > 10 else 0
            cache["stablecoin_mint"] = {"value": signal, "ts": now}
            return signal
    except:
        pass
    return 0

def get_btc_seasonality():
    now = time.time()
    if now - cache["btc_seasonality"]["ts"] < 3600:
        return cache["btc_seasonality"]["value"]
    month = datetime.now(timezone.utc).month
    bonus = 0.25 if month in [10, 11, 12, 1, 2, 3, 4] else 0.0
    cache["btc_seasonality"] = {"value": bonus, "ts": now}
    return bonus

def get_large_transactions():
    now = time.time()
    if now - cache["large_tx_count"]["ts"] < 300:
        return cache["large_tx_count"]["value"]
    try:
        df = get_klines(SYMBOL_BN, "15m", 4)
        if df is not None:
            avg = df["trades"].mean()
            last = df["trades"].iloc[-1]
            signal = 1 if last > avg * 1.5 else 0
            cache["large_tx_count"] = {"value": signal, "ts": now}
            return signal
    except:
        pass
    return 0
...
def check_closed_positions(metrics):
    global stats, active_positions, MIN_SCORE, BASE_MIN_SCORE, losses_in_row
    try:
        ...
        for order_id, pos_info in list(active_positions.items()):
            ...
            hold_hours = (time.time() - pos_info["open_time"]) / 3600
            if hold_hours > MAX_HOLD_HOURS:
                if direction == "LONG" and current_price > entry:
                    log.info(f"⏰ Закрываем по времени (>6ч), P&L положительный")
            ...
            if not still_open:
                ...
                if total_pnl != 0:
                    ...
                    if is_win:
                        ...
                        result = "TP"
                        # Возврат MIN_SCORE после прибыльной сделки
                        ...
                        result = "SL"
                        losses_in_row += 1
                        # Повышение MIN_SCORE после 3 убытков
                        if losses_in_row >= 3 and MIN_SCORE < BASE_MIN_SCORE + 0.5:
                            MIN_SCORE += 0.5
                            log.info(f"⚠️ 3 убытка подряд — MIN_SCORE повышен до {MIN_SCORE}")
                            send_telegram(f"⚠️ <b>3 убытка подряд</b>\nMIN_SCORE повышен до {MIN_SCORE}")
...
def get_signal(df, funding, ob, btc_mom, btc_dir):
    global ob_history, last_ob, yesterday_high, yesterday_low, force_test_done, xgb_model, red_news_until
    ...
    if "close" in df.columns and len(df) >= 2:
        ...
        prev_vol = df.iloc[-2]["volume"] if "volume" in df.columns else 0
        price_drop = (price - prev_price) / prev_price
        if price_drop < -RED_NEWS_DROP and row["volume"] > prev_vol * RED_NEWS_VOL:
            red_news_until = time.time() + RED_NEWS_BLOCK
            log.warning(f"🔴 Красные новости! LONG заблокирован на {RED_NEWS_BLOCK//60} мин")
    ...
    grayscale = get_grayscale_premium()
    defi_ratio = get_defi_tvl_ratio()
    github_act = get_github_activity()
    coinglass_oi_val, coinglass_oi_chg = get_coinglass_oi()
    cme_gap = get_cme_gap()
    stable_mint = get_stablecoin_mint()
    btc_season = get_btc_seasonality()
    large_tx = get_large_transactions()
    ...
    eth_dir = 1 if price > df.iloc[-2]["close"] else -1
    ...
    if xgb_model is not None:
        try:
            features = [[L, S, fear_greed, long_short, taker_ratio, oi_change, cb_premium,
                         eth_btc_chg, liq_diff, gas_price, btc_dom_change, adx, ema50_200_diff,
                         obv_div, whales_val, news_sent, funding_avg]]
    ...
    return direction, entry, sl, tp, score, metrics, f"L:{L:.1f} S:{S:.1f} | ADX:{adx:.1f}"
...
def run_scan():
    ...
    log.info(f"ETH:{price:.2f} | {direction or 'нет'} балл:{score:.1f}/7 | MIN_SCORE:{MIN_SCORE}")
    ...
    if now - last_heartbeat_time >= HEARTBEAT_INTERVAL:
        ...
        send_telegram(
            f"❤️ <b>Heartbeat v4.8</b> {important_str}\n\n"
            f"💰 ETH: <b>{price:.2f}</b>\n"
...
def bot_loop():
    ...
    send_telegram(
        f"🚀 <b>OKX Scalp Bot v4.8</b>\n\n"
        f"🎭 Режим: {'🧪 ТЕСТ' if FORCE_TEST else '⚔️ БОЕВОЙ'}\n"
        f"⚙️ Плечо: x{LEVERAGE} | Сделка: {ORDER_USDT} USDT\n"
        f"🎯 Базовый MIN_SCORE: {BASE_MIN_SCORE}/7 (diff ≥ {BASE_MIN_SCORE_DIFF})\n"
        f"📐 SL: -{int(SL_MARGIN_PCT*100)}% | TP: +{int(TP_MARGIN_PCT*100)}%\n"
        f"🛡️ Защита: красные новости, ликвидации, пятница, Лондон\n"
        f"📊 Статистика: {stats['total']} | ✅ {stats['wins']} | ❌ {stats['losses']} ({winrate:.1f}%)\n\n"
        f"🆕 <b>Новое в v4.8:</b>\n"
        f"• Красные новости — блок LONG\n"
        f"• Сессионный множитель размера\n"
        f"• Автосужение при убытках\n"
        f"• Grayscale, DeFi TVL, GitHub, Coinglass OI\n"
        f"• CME Gap, Stablecoin Mint, BTC Seasonality\n"
        f"• Крупные транзакции ETH"
    )
