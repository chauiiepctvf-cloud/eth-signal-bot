...
# НАСТРОЙКИ
# ─────────────────────────────────────────────
TELEGRAM_TOKEN = os.environ.get("TELEGRAM_TOKEN", "8710461065:AAEtou4YT-j283WLBX2AsouBEX9mQDXsGic")
...
# ЗАГРУЗКА СВЕЧЕЙ
# ─────────────────────────────────────────────
def get_klines(symbol: str = SYMBOL, interval: str = "1h", limit: int = 250):
...
def calculate_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["div_bear"] = price_hh & rsi_lh
    df["div_bull"] = price_ll & rsi_hl
...
def get_funding_data() -> dict:
    ...
    try:
        current = requests.get(f"https://fapi.binance.com/fapi/v1/premiumIndex?symbol={SYMBOL}", timeout=10).json()
        rate = float(current["lastFundingRate"])
        result["rate"] = rate
        
        hist = requests.get(f"https://fapi.binance.com/fapi/v1/fundingRate?symbol={SYMBOL}&limit=8", timeout=10).json()
...
# ─────────────────────────────────────────────
def get_trade_levels(price: float, atr: float, signal_type: str, trade_style: str):
    multipliers = {
        "SCALP": (0.5, 0.5, 0.9),
        "MID": (1.2, 1.2, 2.2),
        "SWING": (2.0, 2.0, 3.5),
...
# АНАЛИЗ ТАЙМФРЕЙМА
# ─────────────────────────────────────────────
def analyze_timeframe(df, funding, oi, tf_name, trade_style, htf_bias="NEUTRAL"):
    default = {
        "signal": "🟡 ЖДИ",
    ...
    if price <= bb_low * 1.005:
        long_score += 2
        long_reasons.append("Цена у нижней BB")
    if ema9 > ema21:
        long_score += 1
    ...
    if div_bull:
        long_score += 2
        long_reasons.append("Бычья дивергенция RSI")
    if vol_ok:
        long_score += 1
    ...
    if price >= bb_top * 0.995:
        short_score += 2
        short_reasons.append("Цена у верхней BB")
    if ema9 < ema21:
        short_score += 1
    ...
    if div_bear:
        short_score += 2
        short_reasons.append("Медвежья дивергенция RSI")
    if vol_ok:
        short_score += 1
    ...
    return {
        "signal": "🟢 ПОКУПКА" if direction == "LONG" else "🔴 ПРОДАЖА",
        "direction": direction,
        "score": score,
        "reason": " | ".join(reasons),
        "entry": entry,
...
def format_message(tf: str, style: str, result: dict, htf_bias: str) -> str:
    ...
    if result["entry"] is not None:
        rr = abs(result["tp1"] - result["entry"]) / max(abs(result["entry"] - result["stop"]), 0.01)
        lines += [
            f"",
