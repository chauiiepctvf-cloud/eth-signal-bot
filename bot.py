...
def okx_place_order(direction: str, entry: float, sl: float, tp: float) -> dict:
    ...
    return okx_place_order_simple(direction, qty)
...
def get_funding_rate() -> float:
    try:
        d = requests.get(f"https://fapi.binance.com/fapi/v1/premiumIndex?symbol={SYMBOL_BN}", timeout=8).json()
...
def run_scan():
    ...
    direction, entry, sl, tp, score, reason = get_scalp_signal(df, funding, ob, force_test=FORCE_TEST)
