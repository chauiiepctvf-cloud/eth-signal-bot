...
return f"OKX Scalp Bot [{mode}] | {datetime.now(timezone.utc).strftime('%d.%m.%Y %H:%M')}
# ══════════════════════════════════════════════
# TELEGRAM
# ══════════════════════════════════════════════
...
log.info("Telegram except Exception as e:
log.error(f"Telegram: {e}")
" if r.status_code == 200 else f"Telegram : {r.text}")
# ══════════════════════════════════════════════
# OKX API
# ══════════════════════════════════════════════
def _ts() -> str:
...
log.error(f"OKX GET {path}: {e}"); return {}
...
log.error(f"OKX POST {path}: {e}"); return {}
# ══════════════════════════════════════════════
...
Шаг 2: выставляем TP+SL через algo order (раздельно — так требует OKX)
"""
okx_set_leverage()
side = "buy" if direction == "LONG" else "sell"
pos_side = "long" if direction == "LONG" else "short"
# Размер: 1 контракт = 0.01 ETH
...
msg = open_r.get("msg", "")
# Детальная ошибка из data[0]
if open_r.get("data"):
msg = open_r["data"][0].get("sMsg", msg)
return {"ok": False, "step": "open", "msg": msg}
order_id = open_r["data"][0].get("ordId", "—")
log.info(f" Позиция открыта | ordId:{order_id} | {direction} {qty}x")
time.sleep(1) # ждём исполнения перед выставлением algo
# ── Шаг 2: TP через algo order ────────────
...
"tpOrdPx": "-1", # рыночная цена при срабатывании
"tpTriggerPxType": "last",
})
tp_ok = tp_r.get("code") == "0"
log.info(f"TP algo: {' ' if tp_ok else ' '} {tp_r.get('msg','')}")
# ── Шаг 3: SL через algo order ────────────
...
log.info(f"SL algo: {' ' if sl_ok else ' '} {sl_r.get('msg','')}")
return {
"ok": True,
"orderId": order_id,
"qty": qty,
...
f"?symbol={SYMBOL_BN}&interval={interval}&limit={limit}",
timeout=10
).json()
...
log.error(f"klines: {e}"); return None
...
L = S = 0
if row["EMA9"] > row["EMA21"] > row["EMA50"]: L += 2
elif row["EMA9"] < row["EMA21"] < row["EMA50"]: S += 2
if 28 < rsi < 45: L += 2
elif 55 < rsi < 72: S += 2
if price > row["VWAP"]: L += 1
else: S += 1
if price <= row["BB_lower"]: L += 2
elif price >= row["BB_upper"]: S += 2
if row["vol_spike"]: L += 1; S += 1
if funding < -0.001: L += 1
elif funding > 0.003: S += 1
if ob > 8: L += 2
elif ob < -8: S += 2
if row["momentum"] > 0 and prev["momentum"] > 0: L += 1
elif row["momentum"] < 0 and prev["momentum"] < 0: S += 1
if row["buy_ratio"] > 0.55: L += 1
elif row["buy_ratio"] < 0.45: S += 1
if L >= MIN_SCORE and L > S: direction, score = "LONG", L
elif S >= MIN_SCORE and S > L: direction, score = "SHORT", S
else: return None, None, None, None, max(L,S), "Балл ниже порога"
if atr > price * 0.02 or atr <= 0:
atr = price * 0.005
entry = price
if direction == "LONG":
sl = round(entry - atr * 0.5, 2)
tp = round(entry + atr * 1.5, 2)
if tp <= entry: tp = round(entry * 1.003, 2)
if sl >= entry: sl = round(entry * 0.997, 2)
else:
sl = round(entry + atr * 0.5, 2)
tp = round(entry - atr * 1.5, 2)
if tp >= entry: tp = round(entry * 0.997, 2)
if sl <= entry: sl = round(entry * 1.003, 2)
log.info(f"{direction} | вход:{entry:.2f} sl:{sl:.2f} tp:{tp:.2f} atr:{atr:.2f}")
return direction, entry, sl, tp, score, f"Балл {score}/{MAX_SCORE} | RSI {rsi:.0f} # ══════════════════════════════════════════════
# ШКАЛА БАЛЛОВ
# ══════════════════════════════════════════════
def score_bar(score) -> str:
filled = round(score / MAX_SCORE * 10)
bar = "█" * filled + "░" * (10 - filled)
emoji = " " if score/MAX_SCORE >= 0.77 else (" return f"{emoji} [{bar}] {score}/{MAX_SCORE}"
" if score/MAX_SCORE >= 0.6 else # ══════════════════════════════════════════════
# ГЛАВНЫЙ ЦИКЛ
# ══════════════════════════════════════════════
last_trade_time = 0
def run_scan():
...
if df is None:
send_telegram(" Ошибка свечей"); return
| OB {
" ")
calc(df)
funding = get_funding()
...
log.info(f"Нет сигнала: {reason}"); return
...
msg.append(" Позиция уже открыта — пропускаю")
else:
res = okx_place_order(direction, entry, sl, tp)
if res["ok"]:
last_trade_time = now
tp_s = " " if res["tp_ok"] else " "
sl_s = " " if res["sl_ok"] else " "
msg += [
...
send_telegram(
f" <b>OKX Scalp Bot</b>\n\n"
f"Режим: {' ТЕСТ (принудит. LONG)' if FORCE_TEST else ' БОЕВОЙ'}\n"
f"Символ: {SYMBOL}\n"
f"Плечо: x{LEVERAGE}\n"
...
f"{' ТЕСТ — ордер откроется принудительно!' if FORCE_TEST else ' Бот торгует авто
)
while True:
try:
run_scan()
